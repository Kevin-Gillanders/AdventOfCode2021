...
impl BingoBoard {
    ...
    fn new_from_vec(board: &Vec<Vec<i32>>) -> BingoBoard {
        BingoBoard { board_data: board.to_vec(), size_of_board: board[0].len() as i32}
    }
    ...
    fn calc_winning_score(&self, drawn_number: &[i32]) ->i32{
        let mut remaining: Vec<i32> = self.board_data.clone().into_iter().flatten().collect();

        let remaining_sum = remaining.into_iter().filter(|num| !drawn_number.contains(num)).sum::<i32>();
    ...
    //     let mut tmp = self.board_data[y][x];
    //     tmp.1 = true;
    //     println!("update cell {} {} -> {:?}", y, x, self.board_data[y][x]);
    //     self.board_data[y][x] = tmp;
    //     println!("updated cell {} {} -> {:?}", y, x, self.board_data[y][x]);
    // }
...
impl BingoGame {
    fn new_from_file(file_name: String) -> BingoGame{
        ...
        let file_content = fs::read_to_string(file_name).expect("There was an issue reading in the file");
        ...
        for (idx, line) in file_content.split('\n').enumerate(){
            if idx == 0{
                ...
                numbers = number_line.into_iter().map(|num| num.parse::<i32>().expect("Incorrect number")).collect();
                continue;
            }
            ...
            let row:Vec<&str> = line.split(' ').map(|x| x.trim()).filter(|x| !x.is_empty()).collect();

            board.push(row.into_iter().map(|num| num.parse::<i32>().expect("Incorrect number")).collect());
    ...
    fn find_winning_board_part2(&self) -> i32{
        ...
        for drawn_number in 0..self.drawn_numbers.len(){
            for (idx, board) in self.bingo_boards.iter().enumerate(){
                if board.is_winning_board(&self.drawn_numbers[0..drawn_number]) && has_won[idx] == false{
                    println!("found {}", win_count);
                    println!("{:?}", &self.drawn_numbers[0..drawn_number]);
                    board.print();
                    println!("its score {}", board.calc_winning_score(&self.drawn_numbers[0..drawn_number]));
